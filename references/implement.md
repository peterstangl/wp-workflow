...
4. **Move any durable conclusion out of the coordination log**, into `CLAUDE.md`, `docs/PLAN.md` or a design document, and leave a pointer behind. Then release every claim the session still holds. This holds whether or not the project tracks its log: a chronological record of an exchange and a living guide are different artefacts, and a conclusion that exists only in the first is one nobody will find when they need it. Where the log *is* excluded, the same step is also the last chance to keep the finding at all.
