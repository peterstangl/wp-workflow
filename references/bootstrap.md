...
1. `CLAUDE.md` and `docs/PLAN.md` (from the drafts) — where they land depends on the Step 2 storage choice. **If that question was never answered** (interview cut short, question skipped), stop and ask it now; never silently fall through to tracking in-repo. Then:
   ...
   The coordination log at `docs/coordination/` inherits whatever mode `docs/` gets. Under **exclude only**, add `/docs/coordination/` beside the `/docs/PLAN.md` entries. Under **symlink**, nothing to do: versioning it in the private repo is the point of that mode. Under **track in this repo**, ask which the user wants — tracked, so the log doubles as a durable record of what happened when and keeps the reasoning behind conclusions that the conclusions themselves do not carry; or added to `.gitignore`, so process traffic stays out of the project's history. Both are defensible, and neither changes the obligation to move durable conclusions out at WP close, which lives in the close-out checklist rather than in the log's mortality.
...
2. A starter `.gitignore` at the repo root, **only if the project uses Python and/or LaTeX** and the repo does not already have a `.gitignore`. Source the content from:
   ...
   If both apply, concatenate them with a clear section separator (e.g. `# --- Python ---` and `# --- LaTeX ---` headers) so the origin of each block stays obvious when the user edits it later. If neither applies, skip *those* blocks. If a `.gitignore` already exists, leave the existing content alone and mention in the summary that it was preserved — the user can merge by hand. The `.gitignore` is always tracked publicly; the privacy logic lives in `.git/info/exclude`, not here, so a public `.gitignore` doesn't advertise the private files' existence.

   **One block is unconditional: [templates/gitignore/coordination.gitignore](templates/gitignore/coordination.gitignore).** It excludes `docs/coordination/heartbeat/`, and it is *not* about the project's language, so it applies even when neither Python nor LaTeX does — create a `.gitignore` containing only it rather than skipping the file — and it is **appended** to a pre-existing `.gitignore` as a clearly delimited block rather than being skipped under "leave it alone". Appending a marked block is not the merge-by-hand case; rewriting someone's file is. Skip silently if a rule already covers the path.

   This is a backstop, not the mechanism. The rule properly belongs to whichever session *creates* the heartbeat directory — see the heartbeat rule in [parallel-sessions.md](parallel-sessions.md) — because coordination is retrofitted into repos that were bootstrapped long before it existed, and bootstrap never runs again for those.
